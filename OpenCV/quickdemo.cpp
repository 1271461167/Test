...
void QuickDemo::canny_Demo(Mat& image)
{
	...
	/*for (int i = 0; i < M1.rows; i++)
	{
		for (int j = 0; j < M1.cols; j++)
		{
			try
			{
				dst.at<uchar>(i, j) = cvRound(sqrt(pow(M1.at<uchar>(i, j), 2) + pow(M2.at<uchar>(i, j), 2)));
			}
